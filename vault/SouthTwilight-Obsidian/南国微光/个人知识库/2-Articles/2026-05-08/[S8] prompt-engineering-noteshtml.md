---
title: prompt-engineering-notes.html
...
relevance: 8
level: l2
hash: 64170224b9ed1d2a
---
# prompt-engineering-notes.html

> Source: [web](https://www.aneasystone.com/archives/2024/01/prompt-engineering-notes.html)

## Summary

```json
{
  "tldr": "系统介绍提示工程定义、核心原则、常用框架及关键技术（如零/少样本、指令微调、思维链），旨在优化大模型输出效果。",
  "summary": "提示工程是一门开发和优化提示词的学科，旨在提升大模型在复杂任务中的表现及与生态工具的对接能力。提示词通常由指令、上下文、输入数据和输出指示组成，其设计需遵循从简入繁、指令明确、避免否定描述等基本原则。文章介绍了CRISPE和结构化提示词等框架，帮助规范提示词编写。核心技术包括零样本与少样本提示，后者利用上下文学习通过示例提升模型理解力，但需注意示例选择与顺序偏差。指令提示通过指令微调（如RLHF）使模型更好理解意图，常与少样本提示结合使用。思维链技术通过展示中间推理步骤，显著增强了大模型在算术和符号推理任务上的能力，且通常在大型模型上效果更佳。",
  "key_points": [
    "提示词由指令、上下文、输入数据和输出指示四要素组成，设计时应遵循从简单开始、使用分隔符、减少模糊描述及明确输出格式等原则。",
    "少样本提示利用上下文学习，通过提供示例提升模型表现，但需注意多数标签偏差、

## Tags

#llm #prompt-engineering #tutorial

## 原文内容

在之前的笔记中，我们学习了很多大模型的使用技巧，比如 实现一个划词翻译插件、实现基于文档的问答助手、实现基于数据库的问答助手 等等，在这些使用场景中，我们应该都或多或少听过 提示工程（Prompt Engineering） 这个概念；另外，在 大模型应用开发框架 LangChain 学习笔记（二） 这篇笔记中，我们学习了什么是 智能体（Agent），并使用 LangChain 实现了几种不同类型的智能体，将提示工程技术发挥得淋漓尽致。那么到底什么是提示工程呢？提示工程又有哪些使用技巧呢？这篇笔记就来系统地学习下相关知识。
什么是提示工程
...
这些指令模型都有对应的论文：
- FLAN - Wei et al. 2021, Finetuned Language Models Are Zero-Shot Learners
...
- TK-Instruct - Wang et al. 2022, Super-NaturalInstructions: Generalization via Declarative Instructions on 1600+ NLP Tasks
...
- 在生成多个推理路径时，一般将模型的温度值设置为 0.5，因为这个值如果设置过小会导致答案基本都一样，过大又会导致答案全都不一样，都会影响到最终的效果；
- 需要生成多少个推理路径（也就是采样次数）也是一个问题，从论文结果来看，候选样本数越多，最终效果越好，论文中一共采样了 40 次，但在实际应用中不可能这样豪横，一般采样 5 次以上就能超过普通的思维链提示；
...
- 问题拆解（Problem Reducing）：第一步自上而下的分解问题，引导模型把问题拆分成子问题；
- 子问题有序解答（Sequentially Solve Subquestions）：第二步自下而上的依次解决问题，逐一回答子问题，并把子问题的回答作为下一个子问题回答的上下文，循序渐进地解决问题，直到给出最终答案；在这个依次回答问题的过程中，问题由少变多，这也是 Least-to-Most 一词的来源。
思维树 (ToT)
...
- 状态评估器（State evaluator） - 如何启发性地评估状态
给定不同的当前状态，让状态评估器评估它们对于解决问题的帮助，以确定哪些状态值得继续探索，以及以何种方式探索；
- 搜索算法（Search algorithm） - 使用什么搜索算法
...
- 给定一组输入输出对形式的示例数据，让 LLM 生成候选指令，比如：
{{Given desired input-output pairs}}\n\nThe instruction is
； - 对于给定数据集，制定一个打分函数，比如准确率或对数概率，我们希望找到一个指令能够令其最大化；
- 使用迭代的蒙特卡洛搜索方法，通过提供类似语义的变体提示来改进最佳候选者，比如：
...
主要分四个阶段：
- 不确定性评估（Uncertainty Estimation）：针对数据集中的所有问题，向 LLM 重复请求 k 次，通过一定的算法对产生的答案计算不确定性；我们知道 LLM 的输出有一定的随机性，同一个问题得到的结果可能是稳定的也可能是不稳定的，第一步就是要找到数据集中的最不稳定的问题；
- 选择阶段（Selection）：选择不确定性指标最高的问题用于后续标注；
- 标注阶段（Annotation）：人工参与对选择的问题进行标注；
- 推理阶段（Inference）：使用这些人工标注的问题作为 CoT 的示例进行推理；
...
整个流程如下：
- 首先通过人工标注的数据（输入和刺激）训练出一个策略模型，这个策略模型可以很小，比如 T5；
- 根据用户输入使用策略模型生成刺激，作为指导下游 LLM 的提示；
- 将生成的刺激与原始输入相结合，作为下游 LLM 的输入，以引导其向刺激的方向生成文本；
- 生成的结果可以通过强化学习对策略模型再次进行训练，使 LLM 与人工偏好更好地结合起来；
...
- 后退提示Step-Back Prompting：AI应对复杂问题的新策略
- 是时候开始讨论大语言模型中的提示架构了吗？
更多
结构化 Prompt
- 如何写好Prompt: 结构化
- LangGPT 结构化提示词
...
应用产品
- Prompt Engineering Tools and IDEs
- Prompted Products
- Open Prompt Studio - AIGC 提示词可视化编辑器
- ChatGPT Shortcut - 让生产力加倍的 ChatGPT 快捷指令
- PromptPerfect - 将您的提示词提升至完美
- LangGPT - Empowering everyone to create high-quality prompts!
...
Prompt Ensembling
- Making Large Language Models Better Reasoners with Step-Aware Verifier - DIVERSE (Diverse Verifier on Reasoning Step)
- Enhancing Self-Consistency and Performance of Pre-Trained Language Models through Natural Language Inference - ConCoRD (Consistency Correction through Relation Detection)
- Ask Me Anything: A simple strategy for prompting language models - AMA Prompting
- Complexity-Based Prompting for Multi-Step Reasoning - 复杂推理提示
- Progressive-Hint Prompting Improves Reasoning in Large Language Models - 渐进提示
- STaR: Bootstrapping Reasoning With Reasoning
数学推理
其他
- Meta-Prompt: A Simple Self-Improving Language Agent - 元提示
- Meta-Prompting: Enhancing Language Models with Task-Agnostic Scaffolding
- Iteratively Prompt Pre-trained Language Models for Chain of Thought - 迭代提示
- Large Language Models are Zero-Shot Rankers for Recommender Systems - 顺序提示
